...
const input = {
  ...
  audit: { auditedAt: "2026-08-04T12:00:00.000Z", pagesAudited: 1, pagesAttempted: 1, durationMs: 100 },
  checks: [
...
describe("generateOpportunity", () => {
  it("uses the configured model, rubric, structured output, no retries, and bounded telemetry", async () => {
    const generate = vi.fn().mockResolvedValue({ output, usage: { inputTokens: 100, outputTokens: 80 } });
    const result = await generateOpportunity(input, { model: "openai/test-model", generate, now: (() => { let time = 0; return () => (time += 25); })() });
    expect(OPPORTUNITY_PROMPT_VERSION).toBe("opportunity-v1");
    expect(generate).toHaveBeenCalledWith(expect.objectContaining({ model: "openai/test-model", maxRetries: 0, timeout: 30_000, temperature: 0 }));
    expect(generate.mock.calls[0]?.[0].system).toContain("addressable sales opportunity");
    expect(generate.mock.calls[0]?.[0].prompt).not.toContain("userId");
    expect(result).toMatchObject({ output, inputTokens: 100, outputTokens: 80, durationMs: 25 });
  });

  it("maps rate limits and invalid evidence to safe typed errors", async () => {
    await expect(generateOpportunity(input, { model: "test", generate: vi.fn().mockRejectedValue(Object.assign(new Error("provider secret"), { statusCode: 429 })) })).rejects.toMatchObject({ code: "RATE_LIMITED" });
    await expect(generateOpportunity(input, { model: "test", generate: vi.fn().mockResolvedValue({ output: { ...output, recommendations: output.recommendations.map((item, index) => index ? item : { ...item, auditCheckKeys: ["invented"] }) }, usage: {} }) })).rejects.toBeInstanceOf(OpportunityGenerationError);
  });
});
