...
export const buildOpportunityInput = ({ prospectName, requestedUrl, audit, checks }: {
  prospectName: string;
  requestedUrl: string;
  audit: { createdAt: Date; pagesAudited: number; pagesAttempted: number; durationMs: number | null };
  checks: Array<{ key: string; category: string; status: string; summary: string; evidence: unknown }>;
}) => ({
