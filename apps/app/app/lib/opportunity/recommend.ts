...
export const selectRecommendations = (
  ...
  const isAppointmentDriven =
    ...
    APPOINTMENT_DRIVEN_CATEGORIES.has(businessCategory);
  ...
  const gatePassed = AFTER_HOURS_COMBINATION.gate.every(
    (key) => byKey.get(key) === "FAIL"
  );
  const eitherPassed = AFTER_HOURS_COMBINATION.either.some(
    (key) => byKey.get(key) === "FAIL"
  );
  if (gatePassed && eitherPassed) {
    for (const key of [
      ...AFTER_HOURS_COMBINATION.gate,
      ...AFTER_HOURS_COMBINATION.either,
    ]) {
      if (byKey.get(key) === "FAIL") {
        add(
          AFTER_HOURS_COMBINATION.service,
          AFTER_HOURS_COMBINATION.weight,
          key
        );
        break;
      }
  ...
  const qualifying = candidates.filter(
    (c) => c.weight >= RECOMMENDATION_THRESHOLD
  );
