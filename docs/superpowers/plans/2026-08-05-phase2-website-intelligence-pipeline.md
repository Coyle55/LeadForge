...
**Naming note:** `PageFacts` already has an `images: number` field (an image *count*, consumed by the existing `image_count` and `image_alt_coverage` checks in `checks.ts:66,130,286-287`). Do not reuse that name — the new field must be called `imageUrls: string[]` or it will silently collide with and corrupt those two existing checks.

In `crawler.ts`, alongside the existing broken-internal-link sampling loop (same file, same `validatePublicTarget` + fetch pattern, same 20-URL sample cap), add `imageUrls: string[]` to `PageFacts` — the resolved `src` URL of each `<img>` tag found during `parseFacts` (the same node list already produces the existing `images`/`imagesWithAlt` counts; add the resolved-URL array alongside them, not instead of them). In the crawl-assembly step, collect these across all sampled pages, dedup, and cap at 20 total, reusing the existing dedup/slice pattern from the broken-link sampler. Count how many of those sampled image URLs return a non-2xx response or fail to fetch, into a new `brokenImages: number` field on `CrawlResult`. Reuse `validatePublicTarget` for each sampled image URL exactly as the broken-link sampler already does — never fetch an image URL without the same SSRF check applied to page URLs.
...
Add an optional `screenshotProvider` dependency (default `noopScreenshotProvider`) to `AuditDependencies`/`runWebsiteAudit`'s options. `runWebsiteAudit` currently throws (via `AuditEngineError`) when the crawl itself fails — e.g. robots-blocked, SSRF-blocked, timeout — so there is no `crawl.finalUrl` to screenshot in that case, and the caller (`apps/app/app/actions/audits.ts`) already marks the whole audit `FAILED` on that path. Only attempt screenshot capture after the crawl has genuinely succeeded (i.e., right before building the return value, using the real `crawl.finalUrl`), wrapped in a `try/catch` that falls back to `{ status: "unavailable", reason: "capture_failed" }` on any thrown error from the provider itself — this satisfies "never block or fail the audit" for the case that actually matters: a successful crawl must never be turned into a failed audit just because screenshot capture errored. Include the result as `screenshot: ScreenshotResult` in `runWebsiteAudit`'s return value.
...
Add to `outreach.test.ts`: `markOutreachSent` sets `status: "SENT"`, `sentAt`, and snapshots the current working `subject`/`body` into `sentSubject`/`sentBody`; sets `Prospect.lastContactedAt`; creates a `ProspectActivity` row with `type: "OUTREACH_SENT"`; advances `pipelineStage` from `NEW` to `CONTACTED` but leaves `INTERESTED`/`PROPOSAL`/`WON`/`LOST` untouched (parameterize this exactly like the existing `it.each` patterns in `pipeline.test.ts`); is owner-scoped and rejects a forged/cross-owner draft ID with the same safe error style as other actions in this file; rejects a draft that is not `COMPLETED` (e.g. `RUNNING`, `FAILED`, or already `SENT`) with the same "not found" style rather than a distinct error message that would leak status information; is atomic (one `$transaction`, matching the existing transaction patterns already used in `pipeline.ts`/`prospects.ts`).
...
Inside one `database.$transaction`: re-fetch the draft and its prospect by ID scoped to `userId` **and `status: "COMPLETED"`** (matching this file's existing convention of scoping fetches by status where the action requires a finished record — see the existing `analysis: { userId, status: "COMPLETED" }` and `status: "COMPLETED"` audit lookups already in this file). Only a `COMPLETED` draft has non-null `subject`/`body` — generation sets both `subject`/`body` and `generatedSubject`/`generatedBody` to the same value in the same update that sets `status: "COMPLETED"`, so there is no separate "was it ever edited" fallback needed: a `COMPLETED` draft's `subject`/`body` are always populated, whether or not the owner has since edited them. If not found (including "found but not `COMPLETED`"), return the same safe "not found" error style used elsewhere in this file.

Then: update the draft to `status: "SENT"`, `sentAt: now`, `sentSubject: draft.subject`, `sentBody: draft.body`; update `Prospect.lastContactedAt = now`, and update `pipelineStage` to `"CONTACTED"` only via a conditional update (`updateMany` with `pipelineStage: "NEW"` in the `where` clause, matching the optimistic-update pattern already used in `pipeline.ts`'s `executeMoveTransaction`) so it never touches a prospect already past `NEW`; create the `ProspectActivity` row with `metadata: { outreachDraftId: draftId }`.
